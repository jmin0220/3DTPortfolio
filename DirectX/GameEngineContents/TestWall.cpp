...
void TestWall::Start()
{
	...
	Wall = CreateComponent<GameEnginePhysicsObject>();
	...
	Wall->GetTransform().SetWorldScale({ 10, 1000, 1000});
	Wall->GetTransform().SetWorldPosition({ 400.0f, 0.0f, 0.0f });

	Wall2 = CreateComponent<GameEnginePhysicsObject>();
