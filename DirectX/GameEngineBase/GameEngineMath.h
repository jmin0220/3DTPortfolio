...
public:
	...
	static float4 QuaternionToEulerAngles(float4 q)
	{
		...
		angles.y = static_cast<float>(std::atan2(sinr_cosp, cosr_cosp));
		...
		if (std::abs(sinp) >= 1)
		{
			...
			angles.x = static_cast<float>(std::copysign(GameEngineMath::PI / 2, sinp)); // use 90 degrees if out of range
		}
